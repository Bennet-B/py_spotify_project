...
9. **Playlist creation moved to `POST /me/playlists`.** The classic `POST /users/{user_id}/playlists` answers 403 for this app (discovered empirically 2026-07-04 during the M2/M3 live Apply test, with correct `playlist-modify-*` scopes on the token). spotipy 2.25's `user_playlist_create` still uses the dead route — `SpotifyClient._sp_create_playlist` therefore raw-calls `sp._post("me/playlists", ...)`, the one documented exception to gotcha #7. `playlist_add_items` (`POST /playlists/{id}/tracks`) works unchanged.
...
- 2026-07-04: M0 merged (PR #5). M1 explore workbench built on `feature/phase2-m1-explore`: insights endpoints (`labels`, `years`, `additions`, `discovery`, `seasonal`, `artists?genre=`, `release-vs-added`) over new pure functions `label_frequencies`/`year_counts`/`artist_track_counts` (+ `track_id` added to `release_vs_added` for the lasso); React explore view — 8 Plotly charts behind an owned `PlotlyChart` wrapper, selection chip bar, cascading genre→artist re-scope, `selectionToRule.ts` mappers (vitest-covered). Verified visually with Playwright (`frontend/.e2e-*.mjs`, gitignored) against the live 3853-track library: genre click → chip + accent + artist re-scope confirmed on screenshots. 180 backend + 13 vitest tests.
- 2026-07-04: M1 merged (PR #6). M2 organizer built on `feature/phase2-m2-organizer`: `organizer.py` rule engine (5 rule kinds incl. DurationRule; AND within bucket, duplicates-allowed or first-match-wins; local files excluded + reported), `create_playlist`/`add_tracks` (100-chunked) + `playlist-modify-*` scopes (one-time re-auth done), pydantic discriminated rule union + `to_core_spec`, preview (pure, 409/400 mapped) + apply job (create-only, batch prefix `[batch] bucket` + description marker) + `BatchStore` history (`.cache/web/batches.json`); frontend Organize tab (bucket cards, rule chips, debounced live preview with coverage/duplicates/overlaps, Apply dialog, batch history) + "Add to bucket" on the selection bar. Live-verified via Playwright: chart click → rule chip → live preview (rock 1617/electronic 901, coverage 62.3%, overlap 116) → Apply dialog. 210 backend + 18 vitest tests.
- 2026-07-04: M2 merged (PR #7). M3 set-analysis + suggest-split on `feature/phase2-m3-analysis`: `set_analysis.py` (overlap_pairs with jaccard/containment, track_membership, unorganized = union(sources) − union(subsets)), `suggest_split.py` (canonical-genre grouping, dominant genres split by decade ranges, first-fit packing, overlap trimming with first-match-wins fallback when duplication stems from multi-genre tracks — found by test), scan job (loads scope cache-first into DatasetStore, typed result via `GET /analysis/scan-result/{job_id}`), sweep endpoint (placeholder playlist, recorded as batch), suggest endpoint returning an OrganizerSpecIn that round-trips into preview; frontend Analyze tab (source/subset scope table, jaccard heatmap, containment callouts, duplication table, unorganized report + sweep) and Suggest-a-split dialog (loads proposal into organizer buckets). Live-verified via Playwright: scan over Liked + 2 subsets, suggestion on 3853 tracks → 6 buckets (3 rock decades, pop, electronic+3, jazz+25) at 97.1% coverage with explanatory notes. 231 backend + 18 vitest tests.
- 2026-07-04: M3 merged (PR #8). Final end-to-end pass found and fixed gotcha #9: the live Apply 403'd because `POST /users/{id}/playlists` is dead — `create_playlist` now raw-calls `POST /me/playlists` (user_id param dropped everywhere). Full real-account verification: UI genre click → bucket → Apply created `[Workbench Demo] Bucket 1` (20 tracks) in the live Spotify account, contents fetched back and confirmed. **Phase 2 M0–M3 complete**; remaining ideas live under Deferred decisions (M4+ stretch).
